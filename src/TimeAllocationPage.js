...
function TimeAllocationPage({ totalHours, allocatedTime, setAllocatedTime, confirmHours }) {
  ...
  return (
    <div className="w-full md:w-[375px] m-auto flex items-center flex-col justify-around h-[100%] overflow-y-scroll">
        <div className="text-4xl font-bold mb-2 h-[70px] w-full justify-center flex items-center">
          <div>Allocated time</div>
        </div>
        <div className="w-11/12 py-2  rounded-xl inset-shadow-sm bg-rose-50">
          <div className="px-1 grid grid-cols-2">
          {allocatedTime.map((phase) => (
            <div key={phase.name} className="flex justify-start items-center my-3 rounded-xl mx-1 p-1 shadow-sm relative max-w-[180px] bg-white">
        ...
        <div className="w-10/12 py-2 flex justify-center items-center text-right">
          <div className="w-1/2">
            <div className="font-semibold">Total time:</div>
            <div className="shadow-sm py-2 bg-gray-100 rounded-xl">{formatTotalTime(hoursSet)}</div>
          </div>
